...
export type OAuthIdentity = {
  ...
  name: string | null;
  provider: OAuthProvider;
  /** Phase 17I-8 — profile enrichment. Provider-supplied avatar URL
   *  (Google `picture` claim) OR a Graph endpoint we'll fetch with
   *  the access token (Microsoft `/me/photo/$value`). When present
   *  the OAuth pipeline downloads + caches the image locally so
   *  expired provider URLs don't produce broken avatars. */
  pictureUrl?: string | null;
  /** Required only when pictureUrl points at an endpoint that needs
   *  Authorization (Microsoft Graph). Omitted for Google. NEVER
   *  persisted; used only for the one-shot avatar fetch. */
  pictureBearerToken?: string | null;
};
...
// the avatar from the provider and store it locally. Runs AFTER the
// session cookie is set so the user lands at /dashboard immediately —
// avatar appears on the next page load (Topbar + Sidebar already
// lazy-fetch /api/auth/me, so no UI changes needed).
//
// Phase 6 rules (verbatim from the spec):
//   • Existing manually uploaded avatars are NEVER overwritten.
//   • OAuth avatars populate only when avatar_url is currently NULL.
//   • Display name MAY refresh — but ONLY when the existing name is
//     blank or matches the email local-part (i.e. was auto-derived
//     during the random signup path). A user who customized their
//     display name keeps it on every subsequent OAuth login.
//   • Tenant isolation preserved — we only touch the resolved user
//     row; no cross-tenant reads or writes.

export type EnrichmentResult = {
  /** Whether we actually wrote a new avatar URL to the user row. */
  avatarUpdated: boolean;
  /** Whether we refreshed the display name. */
  nameUpdated: boolean;
};

export async function enrichUserProfileFromOAuth(args: {
  userId: string;
  identity: OAuthIdentity;
}): Promise<EnrichmentResult> {
  const out: EnrichmentResult = { avatarUpdated: false, nameUpdated: false };

  // Read the row once so we can apply Phase 6 rules atomically vs the
  // current state. (Cheap; the row is hot in cache anyway after the
  // login mint.)
  const user = await db.query.users.findFirst({ where: eq(users.id, args.userId) });
  if (!user) return out;
  ...
  // Only when the current name is blank OR equals the email local-
  // part (i.e. was auto-generated by findOrCreateUserForOAuth on a
  // net-new signup with no `name` in the id_token).
  const currentName = (user.name ?? "").trim();
  const localPart = (user.email ?? "").split("@")[0]?.trim() ?? "";
  const nameLooksAutoDerived =
    currentName.length === 0 ||
    currentName.toLowerCase() === localPart.toLowerCase();
  const newName = (args.identity.name ?? "").trim();
  if (newName && newName !== currentName && nameLooksAutoDerived) {
    updates.name = newName;
    out.nameUpdated = true;
  }

  // ── Avatar fetch + write ──────────────────────────────────────
  // Skip if we already have a non-null avatar_url — preserves manual
  // uploads. Also skip if the provider gave us no pictureUrl.
  if (
    !user.avatarUrl &&
    args.identity.pictureUrl &&
    args.identity.pictureUrl.startsWith("https://")
  ) {
    const stored = await fetchAndStoreAvatar({
      url: args.identity.pictureUrl,
      userId: user.id,
      bearerToken: args.identity.pictureBearerToken ?? undefined,
    });
    if (stored) {
      updates.avatarUrl = stored.publicUrl;
      out.avatarUpdated = true;
    }
  }

  if (Object.keys(updates).length === 0) return out;
  ...
  try {
    ...
    // If the DB update fails we've already written the file. Unlink
    // it to avoid orphans accumulating.
    if (updates.avatarUrl) {
      void unlinkAvatarFile(updates.avatarUrl);
    }
    console.warn(
      `[oauth-enrich] DB update failed (user=${user.id}):`,
      e instanceof Error ? e.message : String(e),
    );
    return { avatarUpdated: false, nameUpdated: false };
  }
