...
/**
 * Business Phone module (P1.2) — the first real "Phone app" surface.
 *
 * Loads /api/tenant/phone/me (capability + masked staff bridge number + usage)
 * and /api/tenant/business-line/calls (recent + missed). Lets an operator:
 *   - see the business number (the caller ID customers see),
 *   - set/update/clear their OWN bridge number (masked after save),
 *   - place an outbound bridge call (New Call / dial pad),
 *   - call back a missed inbound caller.
 *
  ...
  if (meError || !me) {
    return (
      <div className="mx-auto max-w-2xl">
        <Card>
          <CardHeader title="Phone isn't available right now" subtitle="We couldn't load your Business Phone. Please try again." />
          <div className="mt-4">
        ...
        <div className="space-y-5 lg:col-span-2">
          ...
          <Card>
            <div className="flex items-center justify-between">
              <CardHeader title="New call" subtitle="Dial a US or Canada number. The customer sees your business number." />
            ...
            <div className="mt-4 grid gap-5 sm:grid-cols-[1fr_auto]">
              ...
              <div className="flex flex-col items-stretch justify-center gap-3 sm:w-44">
                <Button
                  variant="primary"
                  onClick={onNewCall}
                  disabled={placing || !me.canPlaceCalls}
                  className="h-12"
                >
                  {placing ? (
                    ...
                    <span className="inline-flex items-center gap-2">
                      <PhoneCall className="h-4 w-4" strokeWidth={2} /> Call via Business Phone
                    </span>
                  )}
                </Button>
                {!me.canPlaceCalls && (
