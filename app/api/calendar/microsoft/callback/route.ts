...
export async function GET(req: NextRequest) {
  try {
    ...
    // set at /connect (constant-time, cookie deleted on read). The user is
    // taken from the verified session below, not from `state`.
    if (!(await consumeCalendarStateCookie("microsoft", state))) {
